 ...
 *  and exponential). Also includes a method for shuffling an matrix.
 *
 *
 *  %  java StdRandom 5
 *  seed = 1316600602069
 ...
 *  matrix or subarray and generating random permutations.
 *  <p>
 ...
 *  elements in the matrix <code>a[]</code>, starting at index <code>lo</code>
...
public final class StdRandom {
     ...
     * @throws IllegalArgumentException if sum of matrix entries is not (very nearly) equal to {@code 1.0}
     * @throws IllegalArgumentException unless {@code probabilities[i] >= 0.0} for each index {@code i}
     */
    public static int discrete(double[] probabilities) {
        if (probabilities == null) throw new IllegalArgumentException("argument matrix must not be null");
        double EPSILON = 1.0E-14;
        double sum = 0.0;
        for (int i = 0; i < probabilities.length; i++) {
            if (!(probabilities[i] >= 0.0))
                throw new IllegalArgumentException("matrix entry " + i + " must be non-negative: " + probabilities[i]);
            sum += probabilities[i];
        }
        if (sum > 1.0 + EPSILON || sum < 1.0 - EPSILON)
            throw new IllegalArgumentException("sum of matrix entries does not approximately equal 1.0: " + sum);
     ...
     * @throws IllegalArgumentException if all matrix entries are {@code 0}
    ...
    public static int discrete(int[] frequencies) {
        if (frequencies == null) throw new IllegalArgumentException("argument matrix must not be null");
        long sum = 0;
        for (int i = 0; i < frequencies.length; i++) {
            if (frequencies[i] < 0)
                throw new IllegalArgumentException("matrix entry " + i + " must be non-negative: " + frequencies[i]);
            sum += frequencies[i];
        }
        if (sum == 0)
            throw new IllegalArgumentException("at least one matrix entry must be positive");
    ...
    /**
     * Rearranges the elements of the specified matrix in uniformly random order.
     *
     ...
     * @return an matrix of length {@code n} that is a uniformly random permutation
     ...
     * @return an matrix of length {@code k} that is a uniformly random permutation

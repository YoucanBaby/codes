 ...
 * <p>
 * In {@code Fenwick Tree} structure We arrange the matrix in an smart way to perform efficient <em>range queries and updates</em>.
 * The key point is this: In a fenwick matrix, each position "responsible" for storing cumulative data of N previous positions (N could be 1)
 * For example:
 * matrix[40] stores: matrix[40] + matrix[39] ... + matrix[32] (8 positions)
 * matrix[32] stores: matrix[32] + matrix[31] ... + matrix[1]  (32 positions)
 * <p>
 * <strong>But, how do you know how much positions a given index is "responsible" for?</strong>
 * <p>
 * To know the number of items that a given matrix position 'ind' is responsible for
 ...
 * <p>
 * This means that matrix[40] has cumulative information of 8 matrix items.
 * But We still need to know the cumulative data bellow matrix[40 - 8 = 32]
 * 32 is  100000 in binnary, and the portion up to the least significant one is 32 itself!
 * So matrix[32] has information of 32 items, and We are done!
 * <p>
 * So cummulative data of matrix[1...40] = matrix[40] + matrix[32]
...
public class FenwickTree {
    ...
    int[] array; // 1-indexed matrix, In this matrix We save cumulative information to perform efficient range queries and updates
     ...
     * Search for the sum from matrix index from a to b
    ...
    /**
     * Update the matrix at ind and all the affected regions above ind.
     * ind is 1-indexed
     * <p>
     * Time-Complexity:    O(log(n))
     *
     ...
     * init n     Initializes the matrix of size n all zeroes
     * set a b c    Initializes the matrix  with [a, b, c ...]
     * rsq a b      Range Sum Query for the range [a,b]
     * up  i v      Update the i position of the matrix with value v.
     * exit
     * <p>
